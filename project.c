/****
---------------------------------ReadMe---------------------------------
In this project, 12 nodes send/forward packets using flooding algorithm.
To prevent many copies of packets in circulating, this code implements a 
dampen flooding strategy by using original Link-layer-address-type field: 
1. Sender thread remains same as previous project
2. Receiver thread have changes as below when packet is received:
	i)If the current node is not the packet's destination
	case 1: forward to its neighbor if the neighbor has not been visited	
	case 2: drop the packet if the neighbor has been visited
	This ensures that packet is sended in forward, not backward
	to prevent circulating
	ii)If the current node is the packet's destination
	case 1: if the source is destination's neighbor, parse this direct copy and
	drop other forwarded copies
	case 2: if the source is not destination's neighbor, parse the copy with shortest
	path and drop others
	This decreases the number of copies to be parsed
	
How to run bash script
1. install tmux 
	sudo install apt-get update
	sudo install apt-get tmux
2. run bash script
	bash run.sh
****/
...
/*Convert ip string to ip address in int array*/
void ipstrtoint(char* str, int* arr);

/*Check if source node is current's neighbor*/
int isInNeighbor(int* source, struct neighbor* nei, int num);

/*compare if two ips are same*/
int cmpip(int* ori, int* tgt);

/*Count number of 1 in a char*/
int countOneInChar(char c);

/*Check if current node is alreay in routing*/
int isInRouting(int n, char low, char high);
...
/*Global variable*/
struct node self;  //store self info
int path[13];   //store path cost to/from non-neighbor node
...
int main(int argc, char* argv[]){
	...
	for (int ii = 0; ii < self.numNeighbors; ii++){
		...
		memset(numStr, 0, 3);
		fscanf(confp, "%s", ipStr);
		ipstrtoint(ipStr, nei[ii].fake);
		fscanf(confp, "%s", ipStr2);
		ipstrtoint(ipStr, nei[ii].real);
		fscanf(confp, "%s", numStr);
		nei[ii].neiPort = atoi(numStr); 
	}
	...
	rc = pthread_create(&tid_receiver, NULL, receiver, &args);
	if (rc != 0){
...
int isInRouting(int n, char low, char high){
	int result = -1;
	if (n < 8){
		(low & (1 << n))? (result = 1) : (result = 0);
	}
	else {
		(high & (1 << (n-8)))? (result = 1) : (result = 0);
	}
	return result;
}
...
void* receiver(void *par) {	
    ...
    struct sockaddr_in servaddr, cliaddr, ser_addr; 
	int i, j;
	int ii, jj;
	int ipAdd[4], s_ip[4];
	...
	int recv_nei = 0;   //store the number of packets received from neighbors
	int recv_non = 0;	//store the number of packets received from non-neighbors
	...
	while(1){
		...
		for(i = 0; i < 4; i++){
			...
			s_ip[i] = chartodec(ip->s_Add[i]);
		}
			
		/*parse packet if receiver's ip is same with destination ip*/
		if (cmpip(self.ipAdd, ipAdd) == 0) {
			/*Check if source node is current node's neighbor*/
			isNei = isInNeighbor(s_ip, args->nei, args->num);
			/*Count the number of one*/
			num_one = countOneInChar(lhd->addType[0]) + countOneInChar(lhd->addType[1]);
			/*update path cost from non-neighbor source*/
			if (isNei == 0) {
				if (path[s_ip[3]] > num_one)
					path[s_ip[3]] = num_one;
			}
				
			/*ensure packet with shorter path to parse as follow strategy:
			1. if current node is source node's neighbor, then drop forwarded packets from other nodes
			2. if current node is not source node's neighbor, then select packet with the shortest path*/  
			if (((isNei==1)&&(chartodec(buffer[2])==0))||((isNei==0) && (num_one <= path[s_ip[3]]))) {
			  /*record the number of received packets*/
				if (ipAdd[3] == self.ipAdd[3]) {
					if (isNei == 0){
						recv_non = recv_non + 1;	//update if received from non-neighbors
					}
					else if (isNei == 1) {
						recv_nei = recv_nei + 1; 	//update if received from neighbors
					}		
				}
				printf("The number of packets received from neighbors and non-neighbors are %d, %d\n", recv_nei, recv_non);
				printf("\n\n\n");
				...
				/*Use original Link-layer address type field to track node*/
			...
			/*if destination ip is not this host address, then forward it
			Use Link-layer address type (2 bytes) field to track routing*/
			/*Set the highest bit as flag to forward*/
			buffer[2] = buffer[2] | 128;
			/*Label source node's bit as 1*/
			if (s_ip[3] < 8) {
				buffer[3] = buffer[3] | (1 << s_ip[3]);
			}
			else {
				buffer[2] = buffer[2] | (1 << (s_ip[3]-8));
			}
			/*Label self node's bit as 1*/
			if (self.ipAdd[3] < 8) {
				buffer[3] = buffer[3] | (1 << self.ipAdd[3]);
			}
			else {
				buffer[2] = buffer[2] | (1 << (self.ipAdd[3]-8));
			} 
			...
			for(int ii = 0; ii < args->num; ii++) {
				...
				ser_addr.sin_port = htons(args->nei[ii].neiPort);
				ser_addr.sin_addr.s_addr = INADDR_ANY;
				
				/*Get neighbor's address*/
				int tgt_node = args->nei[ii].fake[3];
				
				int isIn = -1;
				/*Check if current node is already in routing*/
				isIn = isInRouting(tgt_node, lhd->addType[1], lhd->addType[0]);
				
				/*Check if current neighbor is already in routing 
				to ensure routing goes to forward, not backward*/
				if (isIn == 0) {
					/*Label current node's bit as 1*/
					if (tgt_node < 8) {
						buffer[3] = buffer[3] | (1 << tgt_node);
					}
					else {
						buffer[2] = buffer[2] | (1 << (tgt_node-8));
					}
					/*Calculate total length of the packet*/
					int tot = chartodec(ip->tot_len[1]) + chartodec(ip->tot_len[0]) * 16 * 16 + 16;
					/*Forward packet to neighbors*/
					sendto(sockfd, (char *)buffer, tot, MSG_CONFIRM, 
					(const struct sockaddr *) &ser_addr, sizeof(ser_addr));
						
					/*Reset current node's bit as 0*/	
					if (tgt_node < 8) {
						buffer[3] = buffer[3] ^ (1 << tgt_node);
					}
					else {
	...
	for(int i = 0; (packet = pcap_next(pcap, &header)) != NULL; ++i){	
		...
		if ((low == 0) && (high == 8)){
			...
			if (cmpip((args->self).ipAdd, ipAdd) == 0){
				...
				low = 0;
				cp_packet = (unsigned char*)packet;
				cp_packet[2] = cp_packet[0];
				cp_packet[3] = cp_packet[0];
				...
				for(int ii = 0; ii < args->num; ii++) {
					...
					servaddr.sin_port = htons(args->nei[ii].neiPort);
					servaddr.sin_addr.s_addr = INADDR_ANY;
					/*Send one packet to server at a time*/
					sendto(sockfd, (char *)cp_packet, header.len, MSG_CONFIRM, 
